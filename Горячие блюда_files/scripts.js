...
function loadAfterAjax() {
    ...
    $('.order-tabs__item').click(function () {
        ...
        var deliveryId = $(tab).find('input.delivery').val();

        if (parseInt(deliveryId) === 3 || parseInt(deliveryId) === 4 || parseInt(deliveryId) === 5) {
            $('#delivery_price').hide();
            $('#delivery_text').hide();
...
function loadJs() {
    ...
    $(document).on('click', '#authorization', function (e) {
        console.log(123);
        e.preventDefault();
...
$(document).ready(function () {
    ...
    if ($('.delivery-custom__city').length > 0) {
        if ($("#order-delivery-on-home").length > 0) {
            ...
            $('.delivery-custom__city').fias({
                ...
                withParents: true,
                select: function (obj) {
                    $.cookie($(this).prop('id'), $(this).val(), {'path': '/', expires: 1});
                    $.cookie('m_fias_city_id', obj.id, {'path': '/', expires: 1});
                    $('#menu-city_id').val(obj.id);
                    $('#menu-after-city').hide();
                },
                receive: function (obj) {
                    //$(this).fias('controller').setValueById($.cookie('mc_fias_city_id'));
                    var streetId = $.cookie('m_fias_street_id');
                    if (typeof streetId !== 'undefined' && streetId.length > 0 && $('#menu-delivery-custom__street').val().length > 0) {
                        setTimeout(function () {
                            $('#menu-delivery-custom__street').fias('controller').setValueById($.cookie('m_fias_street_id'));
                            $('#menu_fias_street_id').val($.cookie('m_fias_street_id'));
                        }, 1000)
                    }
                }
            });
            ...
            $('.delivery-custom__street').fias({
                ...
                parentInput: '#menu-delivery-custom__city',
                parentId: $('#menu-city_id').val(),
                select: function (obj) {
                    $('#menu_fias_street_id').val(obj.id);
                    $.cookie('m_fias_street_id', obj.id, {'path': '/', expires: 1});
                    $.cookie($(this).prop('id'), $(this).val(), {'path': '/', expires: 1});
                },
                change: function (obj) {
                    $('#menu-delivery-custom__street').val(obj.name);
                    $('.delivery-custom__house').trigger('keyup');
                },
                sendBefore: function (obj) {
                    $('#menu_fias_street_id').val('');
        ...
        if ($("#order-delivery-on-home_cafe").length > 0) {
            $('#menu_cafe-delivery-custom__city').fias({
                type: $.fias.type.city,
                withParents: true,
                select: function (obj) {
                    $.cookie($(this).prop('id'), $(this).val(), {'path': '/', expires: 1});
                    $.cookie('mc_fias_city_id', obj.id, {'path': '/', expires: 1});
                    $('#menu_cafe-street_id').val(obj.id);
                    $('#menu_cafe-after-city').hide();
                },
                receive: function (obj) {
                    //$(this).fias('controller').setValueById($.cookie('mc_fias_city_id'));
                    var streetId = $.cookie('mc_fias_street_id');
                    if (typeof streetId !== 'undefined' && streetId.length > 0 && $('#menu_cafe-delivery-custom__street').val().length > 0) {
                        setTimeout(function () {
                            $('#menu_cafe-delivery-custom__street').fias('controller').setValueById($.cookie('mc_fias_street_id'));
                            $('#fias_street_id').val($.cookie('mc_fias_street_id'));
                            $('#menu_cafe-delivery-custom__street').trigger('change');
                            console.log($('#menu_cafe-delivery-custom__street').val())
                        }, 1000)
                    }
                }
            });

            $('#menu_cafe-delivery-custom__street').fias({
                type: $.fias.type.street,
                parentType: $.fias.type.city,
                parentInput: '#menu_cafe-delivery-custom__city',
                parentId: $('#menu_cafe-street_id').val(),
                select: function (obj) {
                    $('#fias_street_id').val(obj.id);
                    $.cookie('mc_fias_street_id', obj.id, {'path': '/', expires: 1});
                    $.cookie($(this).prop('id'), $(this).val(), {'path': '/', expires: 1});
                },
                sendBefore: function (obj) {
                    $('#fias_street_id').val('');
            ...
            $('#order-delivery-on-home_cafe .delivery-custom__city, #order-delivery-on-home_cafe .delivery-custom__street, ' +
                ...
                inputTimeout = setTimeout(function () {
                    ...
                    $('#menu_cafe-delivery__address-custom').val(cityset);
                    if (cityset) {
                        console.log(cityset);
                        $.ajax({
                            ...
                            type: 'post',
                            data: {
                                'cafe': $('#order-delivery-on-home_cafe select.delivery').val(),
                                address: cityset,
                            ...
                            success: function (response) {
                                $('.hidden_distance').val(response.km);
                                if (response.error == 1) {
                                    //$('#order-errors').show();
                                    ...
                                    if (parseInt(response.near) > 0) {
                                        if (!$('#order-delivery-on-home_cafe .c-select.delivery').length) {
                                            $('#menu_cafe-' + response.near).trigger('click');
                                        } else {
                                            $('#menu_cafe-delivery-' + response.near).prop('selected', true);
                                            $('#order-delivery-on-home_cafe .c-select.delivery + .SelectBox').attr('title', $('#menu_cafe-delivery-' + response.near).html());
                                            $('#order-delivery-on-home_cafe .c-select.delivery + .SelectBox>span').html($('#menu_cafe-delivery-' + response.near).html())
                                        }
                                    }

                                    // LEEFT Купон
                                    var coup = $('#coupon_delivery_home_mc').val();
                                    if (typeof coup !== 'undefined' && coup.length > 0) {
                                        $('#coupon_delivery_home_mc').trigger('keyup'); // LEEFT
                                    }
        ...
        if ($("#order-delivery-on-home-cakes").length > 0) {
            //cakes

            $('#cakes-delivery-custom__city').fias({
                type: $.fias.type.city,
                withParents: true,
                select: function (obj) {
                    $.cookie($(this).prop('id'), $(this).val(), {'path': '/', expires: 1});
                    $.cookie('mc_fias_city_id', obj.id, {'path': '/', expires: 1});
                    $('#cakes-street_id').val(obj.id);
                    $('#cakes-after-city').hide();
                },
                receive: function (obj) {
                    //$(this).fias('controller').setValueById($.cookie('mc_fias_city_id'));
                    var streetId = $.cookie('mc_fias_street_id');
                    if (typeof streetId !== 'undefined' && streetId.length > 0 && $('#cakes-delivery-custom__street').val().length > 0) {
                        setTimeout(function () {
                            $('#cakes-delivery-custom__street').fias('controller').setValueById($.cookie('mc_fias_street_id'));
                            $('#fias_street_id').val($.cookie('mc_fias_street_id'));
                            $('#cakes-delivery-custom__street').trigger('change');
                        }, 1000)
                    }
                }
            });

            $('#cakes-delivery-custom__street').fias({
                type: $.fias.type.street,
                parentType: $.fias.type.city,
                parentInput: '#cakes-delivery-custom__city',
                parentId: $('#cakes-street_id').val(),
                select: function (obj) {
                    $('#fias_street_id').val(obj.id);
                    $.cookie('mc_fias_street_id', obj.id, {'path': '/', expires: 1});
                    $.cookie($(this).prop('id'), $(this).val(), {'path': '/', expires: 1});
            ...
            $('#order-delivery-on-home-cakes .delivery-custom__city, #order-delivery-on-home-cakes .delivery-custom__street, ' +
                '#order-delivery-on-home-cakes .delivery-custom__house, ' +
                '#order-delivery-on-home-cakes select.delivery, ' +
                '#order-delivery-on-home-cakes .delivery-custom__korpus, #order-delivery-on-home-cakes .delivery-custom__flat, .menu_cafe_pickup_cafe_old').change(function () {
                if (inputTimeout != null) {
                    clearTimeout(inputTimeout);
                }

                var city = $('#cakes-delivery-custom__city').val();
                if (city.toLowerCase().indexOf('г.') == -1) {
                    city = 'г.' + city.charAt(0).toUpperCase() + city.substr(1);
                }
                var street = $('#cakes-delivery-custom__street').val();
                var house = $('#cakes-delivery-custom__house').val();

                inputTimeout = setTimeout(function () {
                    var
                        city = $('#cakes-delivery-custom__city').val(),
                        street = $('#cakes-delivery-custom__street').val(),
                        house = $('#cakes-delivery-custom__house').val(),
                        korpus = $('#cakes-delivery-custom__korpus').val(),
                        flat = $('#cakes-delivery-custom__flat').val();
                    ...
                    $('#cakes-delivery__address-custom').val(cityset);

                    var products = new Array();
                    $('.basket-item__list .basket-item__list-item').each(function (i, item) {
                    ...
                    if (cityset) {
                        $.ajax({
                            ...
                            type: 'post',
                            data: {
                                cafe: $('#order-delivery-on-home-cakes select.delivery').val(),
                                address: cityset,
                                price: $('#price_delivery_home_old_cakes_input').val(),
                                type: 'cakes',
                                product: products.join(',')
                            },
                            success: function (response) {
                                var distance = response.km;
                                $('#order-delivery-on-home-cakes .hidden_distance').val(distance);
                                if (response.error == 1) {
                                    //$('#order-errors').show();
                                    //$('#order-errors .error_text').html(response.text);
                                } else {
                                    ...
                                    $("#cakes_delivery_price .price-delivery").html(response.delivery_price);
                                    $('#cakes_delivery_price_input').val(response.delivery_price);
                                    $('#cakes_delivery_price_input').prop('disabled', false);
                                    $('#price_delivery_home_old_cakes').data('price', price + response.delivery_price);

                                    $('#price_delivery_home_old_cakes').html(parseInt(price) + parseInt(response.delivery_price) + '<span class="c-currency-symbol rub">руб</span>');
                                    if (parseInt(response.near) > 0) {
                                        if (!$('#order-delivery-on-home-cakes .c-select.delivery').length) {
                                            $('#cakes-' + response.near).trigger('click');
                                        } else {
                                            $('#cakes-delivery-' + response.near).prop('selected', true);
                                            $('#order-delivery-on-home-cakes .c-select.delivery + .SelectBox').attr('title', $('#cakes-delivery-' + response.near).html());
                                            $('#order-delivery-on-home-cakes .c-select.delivery + .SelectBox>span').html($('#cakes-delivery-' + response.near).html())
                                        }
                                    }

                                    // LEEFT Купон
                                    var coup = $('#coupon_delivery_home_cakes').val();
                                    if (typeof coup !== 'undefined' && coup.length > 0) {
                                        $('#coupon_delivery_home_cakes').trigger('keyup'); // LEEFT
                                    }
        ...
        //one cakes
        if ($("#order-delivery-on-home-oneday_cakes").length > 0) {

            $('#oneday_cakes-delivery-custom__city').fias({
                type: $.fias.type.city,
                withParents: true,
                select: function (obj) {
                    $.cookie($(this).prop('id'), $(this).val(), {'path': '/', expires: 1});
                    $.cookie('mc_fias_city_id', obj.id, {'path': '/', expires: 1});
                    $('#oneday_cakes-street_id').val(obj.id);
                    $('#oneday_cakes-after-city').hide();
                },
                receive: function (obj) {
                    //$(this).fias('controller').setValueById($.cookie('mc_fias_city_id'));
                    var streetId = $.cookie('mc_fias_street_id');
                    if (typeof streetId !== 'undefined' && streetId.length > 0 && $('#oneday_cakes-delivery-custom__street').val().length > 0) {
                        setTimeout(function () {
                            $('#oneday_cakes-delivery-custom__street').fias('controller').setValueById($.cookie('mc_fias_street_id'));
                            $('#fias_street_id').val($.cookie('mc_fias_street_id'));
                            $('#oneday_cakes-delivery-custom__street').trigger('change');
            ...
            $('#oneday_cakes-delivery-custom__street').fias({
                type: $.fias.type.street,
                parentType: $.fias.type.city,
                parentInput: '#oneday_cakes-delivery-custom__city',
                parentId: $('#oneday_cakes-street_id').val(),
                select: function (obj) {
                    $('#fias_street_id').val(obj.id);
                    $.cookie('mc_fias_street_id', obj.id, {'path': '/', expires: 1});
                    $.cookie($(this).prop('id'), $(this).val(), {'path': '/', expires: 1});
            ...
            $('#order-delivery-on-home-oneday_cakes .delivery-custom__city, #order-delivery-on-home-oneday_cakes .delivery-custom__street, ' +
                '#order-delivery-on-home-oneday_cakes .delivery-custom__house, ' +
                '#order-delivery-on-home-oneday_cakes select.delivery, ' +
                '#order-delivery-on-home-oneday_cakes .delivery-custom__korpus, #order-delivery-on-home-oneday_cakes .delivery-custom__flat, .menu_cafe_pickup_cafe_old').change(function () {
                if (inputTimeout != null) {
                    clearTimeout(inputTimeout);
                }

                var city = $('#oneday_cakes-delivery-custom__city').val();
                if (city.toLowerCase().indexOf('г.') == -1) {
                    city = 'г.' + city.charAt(0).toUpperCase() + city.substr(1);
                }
                var street = $('#oneday_cakes-delivery-custom__street').val();
                var house = $('#oneday_cakes-delivery-custom__house').val();

                inputTimeout = setTimeout(function () {
                    var
                        city = $('#oneday_cakes-delivery-custom__city').val(),
                        street = $('#oneday_cakes-delivery-custom__street').val(),
                        house = $('#oneday_cakes-delivery-custom__house').val(),
                        korpus = $('#oneday_cakes-delivery-custom__korpus').val(),
                        flat = $('#oneday_cakes-delivery-custom__flat').val();
                    ...
                    $('#oneday_cakes-delivery__address-custom').val(cityset);
                    ...
                    if (cityset) {
                        $.ajax({
                            ...
                            type: 'post',
                            data: {
                                cafe: $('#order-delivery-on-home-oneday_cakes select.delivery').val(),
                                address: cityset,
                                price: $('#price_delivery_home_old_oneday_cakes_input').val(),
                                type: 'oneday_cakes',
                                product: products.join(',')
                            },
                            success: function (response) {

                                $('#order-delivery-on-home-oneday_cakes .hidden_distance').val(response.km);
                                if (response.error == 1) {
                                    //$('#order-errors').show();
                                    //$('#order-errors .error_text').html(response.text);
                                } else {
                                    ...
                                    var distance = response.km,
                                        price = $('#price_delivery_home_old_oneday_cakes_input').val();
                                    $('#order-delivery-on-home-oneday_cakes .hidden_distance').val(distance);

                                    $(".basket-item__list-item.oneday_cakes .price-delivery").html(response.delivery_price);
                                    $('#oneday_cakes_delivery_price_input').val(response.delivery_price);
                                    $('#oneday_cakes_delivery_price_input').prop('disabled', false);
                                    $('#price_delivery_home_old_oneday_cakes').data('price', price + response.delivery_price);

                                    $('#price_delivery_home_old_oneday_cakes').html(parseInt(price) + parseInt(response.delivery_price) + '<span class="c-currency-symbol rub">руб</span>');
                                    if (parseInt(response.near) > 0) {
                                        if (!$('#order-delivery-on-home-oneday_cakes .c-select.delivery').length) {
                                            $('#oneday_cakes-' + response.near).trigger('click');
                                        } else {
                                            $('#oneday_cakes-delivery-' + response.near).prop('selected', true);
                                            $('#order-delivery-on-home-oneday_cakes .c-select.delivery + .SelectBox').attr('title', $('#oneday_cakes-delivery-' + response.near).html());
                                            $('#order-delivery-on-home-oneday_cakes .c-select.delivery + .SelectBox>span').html($('#oneday_cakes-delivery-' + response.near).html())
                                        }
                                    }

                                    // LEEFT Купон
                                    var coup = $('#coupon_delivery_home_oneday_cakes').val();
                                    if (typeof coup !== 'undefined' && coup.length > 0) {
                                        $('#coupon_delivery_home_oneday_cakes').trigger('keyup'); // LEEFT
                                    }
    ...
    $('.js-mobile-filter-close').click(function () {
        $('.filter-hold-mobile-btn').click();
    })


    $("#popup-programs-sentence__promo").keydown(function (event) {
        setCouponAnimation(this);
    });
    $("#popup-programs-sentence__promo").change(function (event) {
        setCouponAnimation(this);
    });


    function setCouponAnimation($this) {
        var formData = getFormData('pie-order-form');
        formData['action'] = 'enterCoupon';
        formData['withoutcurrency'] = 'y';
        var _this = $this;
            ...
            success: function (e) {
                if (e.error) {
                    ...
                    if (parseInt(e.discount) > 0) {
                        $("#old-price").html(e.old + ' руб');
                        $("#old-price").show();
                    } else {
                        $("#old-price").hide();
                    }
                    $("#total_price").html(e.price + ' руб');
                    $('.errortext').hide();
                    $('input[name="coupon"]').removeClass('error');
                }
            }
    ...
    $(document).on('click', '.icon-basket, .header .symbol-basket, .header .symbol-basket-2, .header-basket__count', function () {
        $('.popup-basket').addClass('popup-slide__show');
        $('.header').addClass('header-top__no-hide');
        // $('html').addClass('lock-html');
    });

    $(document).on('click', '.popup-basket__close', function () {
        $('.popup-basket').removeClass('popup-slide__show');
        $('.header').removeClass('header-top__no-hide');
    });


    $(document).on('click', '#show_main-celebration', function () {
        if ($('.main-celebration').hasClass('hidden')) {
            $('.main-celebration').show();
            $('.main-celebration').removeClass('hidden');
        } else {
            $('.main-celebration').hide();
            $('.main-celebration').addClass('hidden');
        }
    })
});
